...
const registrySchema = Yup.object().shape({
  ...
  Dropdown3: Yup.string().oneOf(
    ["Comprar", "Vender"],
    "Selecione uma opção válida",
  ),
});

const formSuccessPageUrl = `${process.env.NEXT_PUBLIC_FRONT_URL}/forms/imovel/sucesso.html?back=https://www.axpe.com.br/`;
...
function ContactBar() {
  ...
  const { currentBuilding, contactBarActive, contactBarForced, searchFunnel } =
  ...
  function computeIframeUrl(building, searchFunnel, iframes) {
    ...
    const scored = baseList
      .map((f) => {
        ...
        const isMatch =
          fSource === bSource ||
          (fSource === "praia-campo" &&
            ["praia", "campo", "montanha"].includes(bSource)) ||
        ...
        if (!searchFunnel?.finality) {
          if (
            (f.finality === "venda" && building.values?.sell) ||
            (f.finality === "aluguel" && building.values?.rent)
          )
            score += 2;
        }
    ...
    const areaUseful =
      building?.infos?.areaTotal ?? building?.infos?.areaUsefulStart ?? null;
    ...
    const paramsObj = {
      ...
      areaUseful: Number.isFinite(Number(areaUseful))
        ? parseInt(String(areaUseful), 10)
        : areaUseful,
      bedrooms:
        building?.infos?.bedrooms ?? building?.infos?.bedroomsStart ?? null,
      parking:
        building?.infos?.parking ?? building?.infos?.parkingStart ?? null,
      value: null,
      isExclusive: String(building.label?.isExclusive),
      rent: String(building.values?.rent || 0),
      sell: String(building.values?.sell || 0),
      hasSell:
        building.values?.sell > 0 || building.values?.release > 0
          ? "true"
          : "false",
      infoType: building.infos?.type || "",
      url: typeof window !== "undefined" ? window.location.href : "",
      redirectUrl: `${process.env.NEXT_PUBLIC_FRONT_URL}/forms/imovel/sucesso.html`,
    };
  ...
  useEffect(() => {
    ...
    setIsBuilding(
      router.pathname.startsWith("/[category]/[slug]") ||
        (contactBarActive && contactBarForced),
    );
  ...
  useEffect(() => {
    ...
    form.action = qs
      ? `${form.dataset.baseAction}${
          form.dataset.baseAction.includes("?") ? "&" : "?"
        }${qs}`
      : form.dataset.baseAction;
  }, [contactBarActive]);
  ...
  let message =
    `Olá, gostaria de saber mais sobre o imóvel {reference}{areaTotal}{areaUseful}{bedrooms}{parking}. ` +
    pageUrl;
  ...
  if (isBuilding && currentBuilding) {
    ...
    message = message.replace(
      ...
      currentBuilding.infos.areaTotal
        ? ", com " + currentBuilding.infos.areaTotal + " m²"
        : "",
    );
    message = message.replace(
      "{areaUseful}",
      currentBuilding.infos.areaUsefulStart
        ? ", com " + currentBuilding.infos.areaUsefulStart + " m²"
        : "",
    );
    message = message.replace(
      "{bedrooms}",
      currentBuilding.infos.bedrooms
        ? ", " +
            currentBuilding.infos.bedrooms +
            (parseInt(currentBuilding.infos.bedrooms) > 1
              ? " quartos"
              : " quarto")
        : "",
    );
    message = message.replace(
      ...
      currentBuilding.infos.parking
        ? " e " +
            currentBuilding.infos.parking +
            (parseInt(currentBuilding.infos.parking) > 1 ? " vagas" : " vaga")
        : "",
    );
  }
      ...
      {!isBuilding && (
        <>
          <LinkFloat
            ...
            href={
              !isBuilding
                ? `https://wa.me/5511932062653`
                : `https://wa.me/5511932062653?text=${message}`
            }
            target="_blank"
            id="float-contactBar"
          >
            ...
            {isBuilding ? (
              iframeUrl ? (
                ...
                <div style={{ padding: "2rem", textAlign: "center" }}>
                  Preparando formulário do imóvel…
                </div>
              )
            ) : (
              <>
                <Header isBuilding={isBuilding}>
                  ...
                  <h3>Quer comprar ou vender um imóvel?</h3>
                </Header>
                <Column>
                  <p>
                    Deixe seu contato para nosso time entrar em contato com
                    você.
                  </p>
                  <Form
                    ref={refForm}
                  ...
                  >
                    ...
                    <input
                      type="hidden"
                      name="Dropdown"
                      value="AXPE - SITE AXPE"
                    />

                    <input
                      type="hidden"
                      name="zf_referrer_name"
                      value="Form Contato Home"
                    />
                    <input
                      type="hidden"
                      ...
                      onClick={async () => {
                        ...
                        if (!hasErrors && refForm.current) {
                          const normalized = normalizePhone(
                            values.PhoneNumber_countrycode,
                            { allowPlus: true, maxDigits: 15 },
                          );
                          await setFieldValue(
                            "PhoneNumber_countrycode",
                            normalized,
                          );
                          const input = refForm.current.querySelector(
                            "#PhoneNumber_countrycode",
                          );
                    ...
                    <ButtonQuickCall>
                      <a
                        ...
                        target="_blank"
                        className="moreinfo-btn holos-product-contact-method"
                        data-label="Telefone"
                      >
